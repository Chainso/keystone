...
- The live project-management loop is now real only for the shell/sidebar, `New project`, `Project settings`, and the `Runs` index. `Documentation` and `Workstreams` still rely on scaffold selectors and must keep explicit compatibility states for non-scaffold live projects until a later plan cuts them over.

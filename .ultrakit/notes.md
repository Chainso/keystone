...
- Broad `rtk npm test` is still unreliable inside the Codex sandbox on this host because `tests/scripts/demo-contracts.test.ts` can fail with `listen EPERM 127.0.0.1`; use an escalated or host rerun when final proof matters.
...
- `think_live` now executes project-backed compiled DAGs against the full materialized project workspace, including multi-component projects, and `RunWorkflow` fans out the union of `active` and `ready` tasks on each scheduler poll instead of serializing one ready root at a time.
...
- Compile is now document-first and no longer needs a project-level compile target. Multi-component projects can compile and run through `think_live`, while `scripted` intentionally remains single-component only.

...
- The live project-management loop now covers the shell/sidebar, `New project`, `Project settings`, the `Runs` index, live run `Execution` task drill-in, and `Workstreams`; `Documentation` remains scaffold-backed and should keep its explicit compatibility state for non-scaffold live projects.

...
- Post-fix route correction: aligned the canonical `TASK-021` and `TASK-019` workstream rows with matching run-task route ids in [ui/src/features/runs/run-scaffold.ts](../../../ui/src/features/runs/run-scaffold.ts) and [ui/src/features/workstreams/workstreams-scaffold.ts](../../../ui/src/features/workstreams/workstreams-scaffold.ts), then extended [ui/src/test/phase3-destinations.test.tsx](../../../ui/src/test/phase3-destinations.test.tsx) to cover those concrete task-detail routes.
...
- Validation passed for the implementation, targeted fix pass, and final route-target correction: `rtk npm run lint`, `rtk npm run typecheck`, `rtk npm run test -- ui/src/test/phase3-destinations.test.tsx` (`5` tests passed), and `rtk npm run test` (`33` files passed, `2` skipped; `144` tests passed, `8` skipped).

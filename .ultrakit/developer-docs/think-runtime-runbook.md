...
Use this runbook when you need to exercise the shipped Think-backed fixture path end to end.
...
Run the exact Phase 5 validation path from repo root:
...
- `demo:run` sends `X-Keystone-Agent-Runtime: think`, creates the fixture-backed run, and polls until the run reaches `archived` with at least one `run_summary` artifact
